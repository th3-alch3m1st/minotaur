...
def do_work(conn, ch, delivery_tag, body):
    ...
    if opt[0] == 'crithit':
        ...
        input_file = open(filepath + '/alive-' + opt[1] + '.' + opt[2], 'rb')
        with open(filepath + "/crithit" + domain + "." + date, "wb") as out:
            crithit_process = Popen(['/usr/sbin/crithit', '-w', '/tools/input/wordlist.txt', '-t', input_file], stdout=out)
            crithit_process.communicate()[0]
            crithit_process.wait()
            out.flush()
            os.fsync(out)
            out.close()
